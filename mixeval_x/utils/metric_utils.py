...
from mixeval_x.utils.judge_freeform_parser import (
    ChatGPTJudgeCloseendFreeform, 
    ClaudeJudgeCloseendFreeform, 
    GeminiJudgeCloseendFreeform
    )
from mixeval_x.utils.judge_multichoice_parser import (
    ChatGPTJudgeCloseendMultichoice,
    ClaudeJudgeCloseendMultichoice,
    GeminiJudgeCloseendMultichoice
    )
from mixeval_x.utils.judge_text2image import (
    ChatGPTJudgeText2Image,
    ClaudeJudgeText2Image,
    GeminiJudgeText2Image
    )
from mixeval_x.utils.judge_text2action import (
    ChatGPTJudgeText2Action,
    ClaudeJudgeText2Action,
    GeminiJudgeText2Action
    )
from mixeval_x.utils.judge_image2action import (
    ChatGPTJudgeImage2Action,
    ClaudeJudgeImage2Action,
    GeminiJudgeImage2Action
    )
from mixeval_x.utils.common_utils import (
...
def eval_text2image(args, tasks):
    if 'gpt' in args.judge_model.lower():
        model_judge = ChatGPTJudgeText2Image(args)
    elif 'claude' in args.judge_model.lower():
        model_judge = ClaudeJudgeText2Image(args)
    elif 'gemini' in args.judge_model.lower():
        model_judge = GeminiJudgeText2Image(args)
    else:
        raise ValueError(f"Text2Image judge {args.judge_model} not supported.")
    
    tasks_judged_p = model_judge.annotate_parallel(tasks)
    
    assert len(tasks_judged_p) == len(tasks), \
        f"The number of tasks judged is not equal to the number of input tasks. {len(tasks_judged_p)} vs {len(tasks)}"
    
    return tasks_judged_p

def eval_text2action(args, tasks):
    if 'gpt' in args.judge_model.lower():
        model_judge = ChatGPTJudgeText2Action(args)
    elif 'claude' in args.judge_model == 'Claude'.lower():
        model_judge = ClaudeJudgeText2Action(args)
    elif 'gemini' in args.judge_model == 'Gemini'.lower():
        model_judge = GeminiJudgeText2Action(args)
    else:
        raise ValueError(f"Text2Action judge {args.judge_model} not supported.")
...
def eval_image2action(args, tasks):
    if 'gpt' in args.judge_model.lower():
        model_judge = ChatGPTJudgeImage2Action(args)
    elif 'claude' in args.judge_model == 'Claude'.lower():
        model_judge = ClaudeJudgeImage2Action(args)
    elif 'gemini' in args.judge_model == 'Gemini'.lower():
        model_judge = GeminiJudgeImage2Action(args)
    else:
        raise ValueError(f"Image2Action judge {args.judge_model} not supported.")
